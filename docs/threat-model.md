...
- Audit cleanup candidates without mutating files so stale or duplicated memory
...
- Allow the host to expose a narrowed toolset so ordinary profiles do not carry
  unnecessary write-tool definitions.
